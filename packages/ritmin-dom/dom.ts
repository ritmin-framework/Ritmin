...
export default class Dom {
    ...
    public revise(
        ...
        elementAll?: { type: 'div' | 'p' | 'span' | 'a' | 'button' | 'img'; id: string; className: string; content?: string; src?: string } | Array<{ type: 'div' | 'p' | 'span' | 'a' | 'button' | 'img'; id: string; className: string; content?: string; src?: string }>
    ) {
        const validElementTypes: Array<'div' | 'p' | 'span' | 'a' | 'button' | 'img'> = ['div', 'p', 'span', 'a', 'button', 'img'];
        ...
        if (elementAll) {
            if (Array.isArray(elementAll)) {
                elementAll.forEach(item => {
                    if (validElementTypes.includes(item.type) && typeof item.id === 'string' && item.id.trim() !== '' && typeof item.className === 'string') {
                        ...
                        console.error(`Invalid additional element: ${item.type}, ${item.id}, ${item.className}`);
                    }
                });
            } else {
